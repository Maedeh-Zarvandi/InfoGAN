...
class AuxDisc(DiscMLP):
    def __init__(self, n_blocks, mlp_dim, in_dim, dropout, sn, w_init, cont_dim):
        super().__init__(n_blocks, mlp_dim, in_dim, dropout, sn, w_init,)
        self.linear = nn.Linear(in_features=mlp_dim, out_features=cont_dim)
        if w_init is not None: w_init(self.linear.weight)
