...
class BigBiGAN(GAN):
    def __init__(self, generator, encoder, latent_discriminator, img_discriminator, comb_discriminator, fixed_noise, add_noise, aux_discriminator):
        super().__init__()
    ...
    def forward(self, img_real, img_gen, z_noise, z_img, cls):
        ...
        img_real_out, img_real_score = self.img_discriminator(x=img_real + noise, cls=None)
        img_gen_out, img_gen_score = self.img_discriminator(x=img_gen + noise, cls=None)
        ...
        aux_real_cont = self.aux_discriminator(latent=z_img_out, img=img_real_out)
        aux_gen_cont = self.aux_discriminator(latent=z_noise_out, img=img_gen_out)
        ...
        output = {
            ...
            "aux_real_cont": aux_real_cont,
            "aux_gen_cont":aux_gen_cont,
            "cls": cls
        }
    ...
    def classify(self, img):
        latent_hat = self.generate_latent(img)
        pred_cont = self.aux_discriminator(img, latent_hat)  # two times bigger img_input
        return pred_cont
    ...
    def from_config(cls, config):
        ...
        return cls(
            ...
            aux_discriminator=discriminators.AuxDisc.from_config(config),
